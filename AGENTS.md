...
## 安检门/资产处理/AI/视频（文档入口）
- 术语约定：**安检门** = “媒体入库扫描 + 资产处理（缩略图/元数据）+ AI（标签/向量/人脸等）”的端到端后台流水线（原文档/旧口径中曾称为“扫描”）。
- “安检门”的真实语义、资产处理与 AI（标签/向量/人脸等）如何触发与落库、以及**视频如何通过抽帧复用图像模型**：统一以 `md文档/资产处理与扫描设计记录.md` 为准。
...
### 1. Samba/SMB 连接
- **协议**: `smb://`
- **地址**: `smb://10.203.230.77` 
- **用户名**: `wang`
- **密码**: `0000`
## web前端
- 使用http://localhost:3000/开发/调试功能，前端直接指向后端 10.203.230.77:8000
## 你必须做的事情
