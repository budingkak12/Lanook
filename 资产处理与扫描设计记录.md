# 资产处理与“扫描”设计记录（2025-11）

> 本文是一次对话驱动的设计/实现记录，用于说明当前“扫描”在后端的真实语义，以及标签/人脸等 AI 模块在初始化与媒体路径变更时如何被统一触发。

## 一、术语统一：什么是“扫描”

在本项目中，“扫描”不再指某个单独的按钮或接口，而是一个**后台流水线的总称**，包括：

1. **枚举文件并入库**
   - 通过 `MediaSource` + 扫描任务，将文件写入 `Media` 等表。
2. **资产处理模块**
...
**没有一个“开始扫描”按钮**；所有扫描行为都是上述两个入口触发的后台流水线。

## 二、初始化流程：对“所有媒体路径”做全库处理
...
因此，从初始化入口看，“扫描”= **全库导入 + 资产预热 + CLIP + 标签 + 人脸**，并且会自动持续处理，不依赖手工操作。

## 三、设置里新增媒体路径：只对“新增来源”导入，但 AI 模块视全库为总仓库
...
对于 `SourceType.LOCAL` / `SourceType.SMB`：
...
## 七、接口与术语清理：下线 /scan/start，去“按钮化”的扫描概念

为避免误导“有一个开始扫描按钮”，本次做了以下收敛：
...
3. **文案统一**：
   - `setup_routes.py` 中将“首批扫描失败”改为“首批导入失败”，提示改为“后台持续导入/处理中”。
   - `sources_routes.py` 中所有“跳过扫描”的注释改为“跳过重复导入/处理”。

## 八、后续可选工作

1. 若不再需要 `/scan/status`，可以进一步：
   - 将扫描任务进度完全折叠进 `/tasks/scan-progress` / `/tasks/asset-pipeline`；
   - 或保留 `/scan/status` 仅供 CLI/脚本调试（当前已经不被前端调用）。

2. 若引入 NSFW、视频段级处理等能力：
   - 应该沿用本次模式：在初始化与新增来源时自动触发对应暖机任务；
   - 并保证任务视图中新增 NSFW/视频相关指标。
