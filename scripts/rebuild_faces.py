...
def main():
    ...
    parser.add_argument("--threshold", dest="threshold", type=float, default=0.65, help="余弦相似度阈值")
    ...
    try:
        ...
        print(
            f"完成。目录: {path}, 媒体 {media_count} 张, 人脸 {face_count} 个, 聚类 {cluster_count} 个, 阈值 {args.threshold}, 管线 {version}"
        )
    finally:
        db.close()
