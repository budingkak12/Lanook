...
3. CLIP/SigLIP 向量检索（已完成 2025-11-18）
  ...
  4. 标签补充（wd-vit-tagger-v3 白名单版，已完成 2025-11-18）

  - 目标：细粒度标签 + 兼容真实照片。
  - 交付：WdTagService（SigLIP/transformers 版 + 白名单过滤）、media_tags 新增 source_model/confidence、脚本 `uv run python -m scripts.wd_tag_rebuild`、API `POST /tags/rebuild` 与 `GET /media/{id}/tags`、/smart-search 标签筛选面板。
  - 验证：先 `uv run python -m scripts.init_db --media-path 测试图片`，再运行 `uv run python -m scripts.wd_tag_rebuild --base 测试图片 --model wd-v3 --min-conf 0.4`；完成后 /smart-search 文本检索 + 标签勾选过滤。
